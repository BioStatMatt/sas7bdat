...
32		3	low	*????????????*
35		3	low	bitmasks (SAS_host, SAS_release) Alignment?
38		1	low	*????????????*
...
164 Alignment_	16	high	2x LE double, timestamp, secs since 1/1/60
180		16	low	*????????????*
196		20	low	*????????????*
...
In files generated by 64 bit builds of SAS, 'Align' means that the offset of this data field should be advanced so that the offset is a factor of 8 bytes. For files generated by 32 bit builds of SAS, the alignment is 4 bytes.
